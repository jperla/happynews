...
void read_settings(char* filename)
{
    ...
    int success = 1;
    success = success && fscanf(fileptr, "var max iter %d\n", &VAR_MAX_ITER) > 0;
    success = success && fscanf(fileptr, "var convergence %f\n", &VAR_CONVERGED) > 0;
    success = success && fscanf(fileptr, "em max iter %d\n", &EM_MAX_ITER) > 0;
    success = success && fscanf(fileptr, "em convergence %f\n", &EM_CONVERGED) > 0;
    success = success && fscanf(fileptr, "alpha %s", alpha_action) > 0;
    if (!success)
    {
      printf("Error reading settings from file %s "
