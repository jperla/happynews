...
lda_model* load_lda_model(char* model_root)
{
    ...
    int success = fscanf(fileptr, "num_topics %d\n", &num_topics) > 0;
    success = success && fscanf(fileptr, "num_terms %d\n", &num_terms) > 0;
    success = success && fscanf(fileptr, "alpha %f\n", &alpha) > 0;
    printf("%d %d %f\n", num_topics, num_terms, alpha);
    if (!success)
    {
      printf("Error reading file %s. Failing.\n", filename);
      exit(1);
    }
    fclose(fileptr);
    ...
    sprintf(filename, "%s.beta", model_root);
    printf("loading beta file %s\n", filename);
    fileptr = fopen(filename, "r");
    for (i = 0; i < num_topics; i++)
    {
        for (j = 0; j < num_terms; j++)
        {
            success = fscanf(fileptr, "%f", &x) > 0;
