...
def run_em(data):
    ...
    documents = [doc_to_array(d) for d in documents]
    comments = [doc_to_array(c) for c in comments]
    ...
    while elbo_did_not_converge(elbo, last_elbo, iterations, criterion=0.1, max_iter=20):
...
def update_phi_lda_E_step(text, phi, gamma, beta, y_d, eta, sigma_squared):
    ...
    if isinstance(text, np.ndarray):
        ...
        pB = np.log(beta[:,text].T)
        pD = np.dot(np.matrix(np.sum(eta * phi_minus_n, axis=1)).T, np.matrix(right_eta_times_const))

        phi[:,:] = np.exp(pB + pD + const)
        row_normalize(phi)
    else:
